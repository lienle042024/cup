...
void cupsdCreateCommonData(void)
{
  int i;                   /* Looping var */
  ...
  ippAddStrings(CommonData, IPP_TAG_PRINTER, IPP_TAG_KEYWORD | IPP_TAG_COPY,
                ...
                sizeof(multiple_document_handling) /
                    sizeof(multiple_document_handling[0]),
                NULL,
...
void cupsdDeleteAllPrinters(void)
{
    ...
    int update)         /* I - Update printers.conf? */
{
  ...
  if (p->job)
    cupsdSetJobState(p->job, IPP_JOB_PENDING, CUPSD_JOB_FORCE,
                     update ? "Job stopped due to printer being deleted." : "Job stopped.");
...
void cupsdDeleteTemporaryPrinters(int force) /* I - Force deletion instead of auto? */
{
...
void cupsdLoadAllPrinters(void)
{
  int i;              /* Looping var */
...
void cupsdSaveAllPrinters(void)
{
  int i;                    /* Looping var */
  cups_file_t *fp;          /* printers.conf file */
...
void cupsdSetDeviceURI(cupsd_printer_t *p, /* I - Printer */
...
void cupsdSetPrinterAttrs(cupsd_printer_t *p) /* I - Printer to setup */
{
  int i;                       /* Looping var */
  ...
  if (cupsArrayCount(Banners) > 0)
  {
    ...
    if (attr != NULL)
    {
      attr->values[0].string.text = _cupsStrAlloc(Classification ? Classification : p->job_sheets[0]);
      attr->values[1].string.text = _cupsStrAlloc(Classification ? Classification : p->job_sheets[1]);
    }
  }
    ...
    int update)         /* I - Update printers.conf? */
{
  ...
  if (old_state != s)
  {
    cupsdAddEvent(s == IPP_PRINTER_STOPPED ? CUPSD_EVENT_PRINTER_STOPPED : CUPSD_EVENT_PRINTER_STATE, p, NULL,
...
void cupsdStopPrinter(cupsd_printer_t *p, /* I - Printer to stop */
                      int update)         /* I - Update printers.conf? */
{
  /*
    ...
    const char *filter)      /* I - Filter to add */
{
  ...
  cupsdLogMessage(CUPSD_LOG_DEBUG2,
                  ...
                  "filter=\"%s\")",
                  p, p->name, filtertype, filtertype->super,
      ...
      if (desttype != filtertype)
      {
        ...
        if (!mimeFilterLookup(MimeDatabase, desttype, filtertype))
        {
          cupsdLogMessage(CUPSD_LOG_DEBUG2,
                          ...
                          "0 -",
                          p->name, desttype->super, desttype->type,
    ...
    if (ppd->num_sizes == 0 || !p->pc)
    {
      if (!ppdFindAttr(ppd, "APScannerOnly", NULL) && !ppdFindAttr(ppd, "cups3D", NULL))
        cupsdLogMessage(CUPSD_LOG_CRIT,
                        ...
                        "options and is therefore invalid.",
                        p->name);
      ...
      if (outUrl && icnsFileUrl)
      {
        ...
        if (sourceRef)
        {
          ...
          if (biggestIconRef)
          {
            ...
            imageRef = closestTo128IconRef ? closestTo128IconRef : biggestIconRef;
            CGImageRetain(imageRef);
            CGImageRelease(biggestIconRef);

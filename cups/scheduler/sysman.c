...
void cupsdMarkDirty(int what) /* I - What file(s) are dirty? */
{
...
void cupsdSetBusyState(int working) /* I - Doing significant work? */
{
  int i;               /* Looping var */
  cupsd_job_t *job;    /* Current job */
...
void cupsdStartSystemMonitor(void)
{
  ...
  notify_register_dispatch(kIOPSNotifyPowerSource, &PSToken, dispatch_get_main_queue(), ^(int t) {
    (void)t;
    ACPower = IOPSGetTimeRemainingEstimate() == kIOPSTimeRemainingUnlimited;
  });
}
...
void cupsdStopSystemMonitor(void)
{

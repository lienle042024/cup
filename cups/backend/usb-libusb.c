          ...
          if (protocol > 0)
          {
            ...
            if (!open_device(&printer, data != NULL))
            {
              ...
              if ((*cb)(&printer, device_uri, device_id, data))
              {
                ...
                {
                  printer.read_endp = confptr->interface[printer.iface].altsetting[printer.altset].endpoint[printer.read_endp].bEndpointAddress;
                }
                else
                  fprintf(stderr, "DEBUG: Uni-directional USB communication "
                                  "only!\n");
                printer.write_endp = confptr->interface[printer.iface].altsetting[printer.altset].endpoint[printer.write_endp].bEndpointAddress;
            ...
            int verbose)            /* I - Update connecting-to-device state? */
{
  ...
  if ((errcode =
           libusb_get_config_descriptor(printer->device, printer->conf, &confptr)) < 0)
  {
  ...
  {
    number1 = confptr->interface[printer->iface].altsetting[printer->altset].bInterfaceNumber;
    number2 = confptr->interface[printer->iface].altsetting[printer->altset].bAlternateSetting;

    while ((errcode =
                libusb_set_interface_alt_setting(printer->handle, number1, number2)) < 0)
    {
      if (errcode != LIBUSB_ERROR_BUSY)
      {
        fprintf(stderr,
    ...
    usb_printer_t *printer) /* I - Printer */
{
  ...
  if (libusb_get_config_descriptor(printer->device, printer->conf,
    ...
    interface = confptr->interface[printer->iface].altsetting[printer->altset].bInterfaceNumber;

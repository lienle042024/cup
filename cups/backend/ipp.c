     ...
     char *argv[]) /* I - Command-line arguments */
{
  ...
  if (argc == 1)
  {
    ...
    printf("network %s \"Unknown\" \"%s (%s)\"\n",
           s, _cupsLangString(cupsLangDefault(), _("Internet Printing Protocol")), s);
  ...
  while (!job_canceled && copies_remaining > 0)
  {
    ...
    request = new_request((num_files > 1 || create_job) ? IPP_OP_CREATE_JOB : IPP_OP_PRINT_JOB,
               ...
               (!job_canceled || compatsize > 0))
        {
          ...
          if (FD_ISSET(fd, &input))
          {
            if ((bytes = read(fd, buffer, sizeof(buffer))) > 0)
            {
              ...
              if ((http_status = cupsWriteRequestData(http, buffer, (size_t)bytes)) != HTTP_STATUS_CONTINUE)
                break;
            }
    ...
    _cups_monitor_t *monitor) /* I - Monitoring data */
{
  ...
  while (monitor->job_state < IPP_JSTATE_CANCELED && !job_canceled)
  {
    ...
    if (httpGetFd(http) >= 0)
    {
      ...
      job_op = (monitor->job_id > 0 && monitor->get_job_attrs) ? IPP_OP_GET_JOB_ATTRIBUTES : IPP_OP_GET_JOBS;

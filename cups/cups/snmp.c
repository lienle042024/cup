...
void _cupsSNMPSetDebug(int level) /* I - 1 to enable debug output, 0 otherwise */
{
  _cups_globals_t *cg = _cupsGlobals(); /* Global data */
    ...
    const int *oid)            /* I - OID */
{
  ...
  if (fd < 0 || !address || version != CUPS_SNMP_VERSION_1 || !community ||
      (request_type != CUPS_ASN1_GET_REQUEST &&
       request_type != CUPS_ASN1_GET_NEXT_REQUEST) ||
      request_id < 1 || !oid)
  {
                 ...
                 cups_snmp_t *packet)   /* I - SNMP packet */
{
  ...
  else if ((packet->version = asn1_get_integer(&bufptr, bufend, length)) != CUPS_SNMP_VERSION_1)
  ...
  else
  {
    ...
    if ((packet->request_type = (cups_asn1_t)asn1_get_type(&bufptr, bufend)) != CUPS_ASN1_GET_RESPONSE)

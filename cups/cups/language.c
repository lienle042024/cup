...
void cupsLangFree(cups_lang_t *lang) /* I - Language to free */
{
  _cupsMutexLock(&lang_mutex);
...
cupsLangGet(const char *language) /* I - Language or locale */
{
  ...
  DEBUG_printf(("4cupsLangGet: encoding=%d(%s)", encoding,
                encoding == CUPS_AUTO_ENCODING ? "auto" : lang_encodings[encoding]));
...
void _cupsMessageFree(cups_array_t *a) /* I - Message array */
{
    ...
    cups_encoding_t encoding) /* I - Encoding of locale */
{
  ...
  DEBUG_printf(("7cups_cache_lookup(name=\"%s\", encoding=%d(%s))", name,
                encoding, encoding == CUPS_AUTO_ENCODING ? "auto" : lang_encodings[encoding]));
  ...
  for (lang = lang_cache; lang != NULL; lang = lang->next)
  {
    DEBUG_printf(("9cups_cache_lookup: lang=%p, language=\"%s\", "
                  "encoding=%d(%s)",
                  (void *)lang, lang->language, lang->encoding,

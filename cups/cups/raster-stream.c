...
void _cupsRasterDelete(cups_raster_t *r) /* I - Stream to free */
{
  if (r != NULL)
  {
    if (r->buffer)
    ...
    cups_raster_t *r) /* I - Raster stream */
{
  ...
  else if (r->mode == CUPS_RASTER_WRITE_APPLE)
  {
    ...
    appleheader[1] = r->header.cupsColorSpace == CUPS_CSPACE_SRGB ? 1 : r->header.cupsColorSpace == CUPS_CSPACE_CIELab ? 2
    ...
    return (cups_raster_io(r, (unsigned char *)&(r->header), sizeof(r->header)) == sizeof(r->header));
}

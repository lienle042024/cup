...
void _cupsBufferRelease(char *b) /* I - Buffer to release */
{
...
void ippDelete(ipp_t *ipp) /* I - IPP message */
{
    ...
    ipp_attribute_t *attr) /* I - Attribute */
{
  ...
  case IPP_TAG_RESOLUTION:
    for (i = 0; i < attr->num_values; i++)
    {
      ...
      {
        ipp_set_error(IPP_STATUS_ERROR_BAD_REQUEST, _("\"%s\": Bad resolution value %dx%d%s - bad units value (RFC 8011 section 5.1.16)."), attr->name, attr->values[i].resolution.xres, attr->values[i].resolution.yres, attr->values[i].resolution.units == IPP_RES_PER_INCH ? "dpi" : attr->values[i].resolution.units == IPP_RES_PER_CM ? "dpcm"
                                                                                                                                                                                                                                                                                                                                            : "unknown");
        return (0);
      }
    }
           ...
           int collection) /* I - 1 if a collection, 0 otherwise */
{
  ...
  for (attr = ipp->attrs; attr != NULL; attr = attr->next)
  {
    ...
    DEBUG_printf(("5ipp_length: attr->name=\"%s\", attr->num_values=%d, "
                  "bytes=" CUPS_LLFMT,
                  attr->name, attr->num_values, CUPS_LLCAST bytes));

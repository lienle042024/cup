            ...
            int modify)   /* I - Modify the printer? */
{
  ...
  {
    cgiStartHTML(title);
    cgiShowIPPError(modify ? _("Unable to modify class") : _("Unable to add class"));
  }
  else
  {
    /*
              ...
              int modify)   /* I - Modify the printer? */
{
  ...
  if (!var)
  {
    ...
    else
    {
      ...
      {
        cgiStartHTML(title);
        cgiShowIPPError(modify ? _("Unable to modify printer") : _("Unable to add printer"));
        cgiEndHTML();
        return;
      }
    }
...
do_menu(http_t *http) /* I - HTTP connection */
{
  ...
  if ((val = cupsGetOption("DefaultAuthType", num_settings,
                           settings)) != NULL &&
      !_cups_strcasecmp(val, "Negotiate"))
...
do_set_allowed_users(http_t *http) /* I - HTTP connection */
{
  ...
  else
  {
    ...
    else
    {
      ...
      cgiCopyTemplateLang(is_class ? "class-modified.tmpl" : "printer-modified.tmpl");
    }
               ...
               int is_class) /* I - Set options for class? */
{
  ...
  if (!have_options || ppdConflicts(ppd))
  {
                                 ...
                                 IPP_TAG_ZERO)) != NULL)
    {
      ...
      cgiSetVariable("DEFCHOICE", attr != NULL ? attr->values[0].string.text : "");
      ...
      cgiSetVariable("DEFCHOICE", attr != NULL && attr->num_values > 1 ? attr->values[1].string.text : "");
  ...
  else
  {
    ...
    request = ippNewRequest(is_class ? CUPS_ADD_MODIFY_CLASS : CUPS_ADD_MODIFY_PRINTER);
